...
export class PdfGenerator {
  ...
  private async convertSvgToDataUrl(imgElement: HTMLImageElement): Promise<string> {
    try {
      ...
      console.log('Converting SVG:', svgUrl);
  ...
  async generatePdf(element: HTMLElement, filename: string = 'mtg-divider-cards.pdf'): Promise<void> {
    try {
      ...
      console.log(`Grid: ${rows} rows × ${columns} columns (${cardCount} cards total)`);
      ...
      console.log(`Target PDF size: ${targetWidth}mm × ${targetHeight}mm`);
      ...
      console.log(`Found ${svgImages.length} SVG images to convert`);
      ...
      console.log('Using PDF dimensions:', imgWidth.toFixed(2), 'x', imgHeight.toFixed(2), 'mm');
      ...
      if (imgWidth > maxWidth) {
        ...
        console.log('Scaled down to fit A4:', imgWidth.toFixed(2), 'x', imgHeight.toFixed(2), 'mm');
      }
